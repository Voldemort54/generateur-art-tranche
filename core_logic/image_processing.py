...
import shutil
import math
from datetime import date, datetime # Importe date et datetime (pour datetime.now())
...
def generer_pdf_a_partir_tranches(dossier_tranches_source, hauteur_livre_mm_pdf, largeur_tranche_etiree_cible_mm_pdf,
    ...
    # Le chemin du PDF sera créé dans le dossier des tranches pour un nettoyage facile.
    pdf_filename = f"generated_foreedge_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.pdf" 
    ...
    try:
        ...
        creation_date_formatted = date.today().strftime("%d/%m/%Y") 
